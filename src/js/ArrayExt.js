...
ArrayExt.shuffle = (inputArray) => {
    const outputArray = Array.from(inputArray)
    for (let i = 0; i < inputArray.length; i++) {
        const swapIndex = Math.floor(Math.random() * (inputArray.length - i)) + i
        const tempValue = outputArray[i]
        outputArray[i] = outputArray[swapIndex]
        outputArray[swapIndex] = tempValue
    }
    return outputArray
}
